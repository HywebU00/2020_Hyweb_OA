...
alertPlanInput.forEach((item) => {
  ...
  function inputChangeCardColor() {
    item.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.classList.toggle(
      "is-changeBackground"
    );
  }
});
...
for (let e = 0; e < switchLetterBtn.length; e++) {
  switchLetterBtn[e].addEventListener("click", function () {
    let switchActive =
      switchLetterBtn[e].classList.contains("is-switch-active");
    if (switchActive === false) {
